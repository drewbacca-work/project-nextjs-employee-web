...
export function middleware(request: NextRequest) {
  ...
  const dupLocale = new Set(splitPath).size !== splitPath.length;
  ...
  if (pathLocale) {
    ...
    if (isDefaultLocale) {
      ...
      const remainingPath = request.nextUrl.pathname.replaceAll(
        `/${pathLocale}`,
        ""
      );

      response = NextResponse.redirect(
        new URL(basePath + `/${pathLocale}${remainingPath}`, request.url)
      );
    }
